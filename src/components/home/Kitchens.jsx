        ...
        {kitchens.items.map((item, index) => {
          ...
          return (
            <li key={item.title} data-reveal data-reveal-delay={index * 70}>
              <a
                href={item.href}
                className="group flex h-full min-h-[520px] flex-col overflow-hidden rounded-t-[999px] rounded-b-sm border border-line bg-paper text-center shadow-soft transition duration-300 hover:-translate-y-1 hover:border-gold/55 hover:shadow-lift focus-visible:border-gold"
              >
                ...
                <div className="-mt-9 flex flex-1 flex-col items-center px-6 pb-6">
                  ...
                  <p className="mx-auto mt-5 max-w-[22ch] text-body leading-7 text-muted">{item.text}</p>
